...
int main() {
    ...
    while (m--) {
        ...
        if (op == 1) {
            ...
            double res = query(tree_val, 1, 1, n, x, y) / (y - x + 1);
            printf("%.4lf\n", res);
        } else if (op == 3) {
            double sum = query(tree_val, 1, 1, n, x, y);
            double sum_sq = query(tree_fval, 1, 1, n, x, y);
            double mean = sum / (y - x + 1);
            double variance = (sum_sq / (y - x + 1)) - (mean * mean);
            printf("%.4lf\n", variance);
        }
    }
    return 0;
